...
RET_TYPE logic_smartcard_user_unlock_process(void)
{
    ...
    {
        gui_prompts_display_information_on_screen_and_wait(ID_STRING_LAST_PIN_TRY);
        ...
        if (gui_prompts_get_user_pin(&temp_pin, ID_STRING_INSERT_PIN) == RETURN_OK)
        {
            ...
            switch(temp_rettype)
            {
                ...
                case RETURN_MOOLTIPASS_1_TRIES_LEFT :
                {
                    ...
                    gui_prompts_display_information_on_screen_and_wait(ID_STRING_WRONGPIN1LEFT);
                    if(warning_displayed == FALSE)
                    {
                        // Inform the user to remove his smart card
                        gui_prompts_display_information_on_screen(ID_STRING_REMOVE_CARD);
                ...
                default :
                {
                    ...
                    gui_prompts_display_information_on_screen_and_wait(ID_STRING_WRONGPIN1LEFT + temp_rettype - RETURN_MOOLTIPASS_1_TRIES_LEFT);
                    break;
                }
            }
        }
