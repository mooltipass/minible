...
nodemgmt_data_category_te logic_user_getting_data_category = NODEMGMT_STANDARD_DATA_TYPE_ID;
...
uint16_t logic_user_fav_last_used_service = NODE_ADDR_NULL;
uint16_t logic_user_fav_last_used_login = NODE_ADDR_NULL;
...
void logic_user_invalidate_preferred_starting_service(void)
{
    ...
    logic_user_fav_last_used_service = NODE_ADDR_NULL;
    logic_user_fav_last_used_login = NODE_ADDR_NULL;
...
/*! \fn     logic_user_get_data_from_service(cust_char_t* service, uint8_t* buffer, uint16_t* nb_bytes_written, BOOL is_message_from_usb, nodemgmt_data_category_te data_type)
...
*   \return success or not
*/
RET_TYPE logic_user_get_data_from_service(cust_char_t* service, uint8_t* buffer, uint16_t* nb_bytes_written, BOOL is_message_from_usb, nodemgmt_data_category_te data_type)
{
...
/*! \fn     logic_user_empty_data_service(cust_char_t* service, BOOL is_message_from_usb, nodemgmt_data_category_te data_type)
...
/*! \fn     logic_user_check_data_service(cust_char_t* service, nodemgmt_data_category_te data_type)
...
/*! \fn     logic_user_add_data_service(cust_char_t* service, BOOL is_message_from_usb, nodemgmt_data_category_te data_type)
...
void logic_user_usb_get_credential(cust_char_t* service, cust_char_t* login, BOOL send_creds_to_usb, BOOL get_totp)
{
    ...
    if ((login != 0) || (nb_logins_for_cred == 1))
    {
        ...
        else
        {
            ...
            /* Decrypt TOTPsecret */
            logic_encryption_ctr_decrypt(temp_cnode.cred_child.TOTP.TOTPsecret_ct, temp_cnode.cred_child.TOTP.TOTPsecret_ctr, MEMBER_SIZE(TOTP_cred_node_t, TOTPsecret), FALSE);
...
/*! \fn     logic_user_manual_select_favorite(void)
*   \brief  Logic for finding a given favorite
*/
void logic_user_manual_select_favorite(void)
{
    uint16_t chosen_service_addr = NODE_ADDR_NULL;
    ...
    /* If feature is enabled, start with the last used service */
    node_type_te temp_node_type;
    if ((custom_fs_settings_get_device_setting(SETTINGS_PREF_ST_SERV_FEATURE) != FALSE) &&
        (logic_user_fav_last_used_login != NODE_ADDR_NULL) &&
        (nodemgmt_check_user_permission(logic_user_fav_last_used_login, &temp_node_type) == RETURN_OK) &&
        (temp_node_type == NODE_TYPE_CHILD))
    {
        chosen_service_addr = logic_user_fav_last_used_service;
        chosen_login_addr = logic_user_fav_last_used_login;
    }
    ...
    while (TRUE)
    {
        ...
        {
            gui_prompts_favorite_selection_screen(&chosen_service_addr, &chosen_login_addr);
        ...
        else if (state_machine_index == 1)
        {
            ...
            else
            {
                ...
                logic_user_fav_last_used_login = chosen_login_addr;
                return;
            }
        }
