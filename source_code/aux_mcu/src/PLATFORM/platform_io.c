...
void platform_io_enable_battery_charging_ports(void)
{
    ...
    temp_adc_ctrb_reg.bit.PRESCALER = ADC_CTRLB_PRESCALER_DIV64_Val;                                // Set fclk_adc to 48M / 64 = 750kHz
    ADC->CTRLB = temp_adc_ctrb_reg;                                                                 // Write ctrlb
    ADC->AVGCTRL.reg = ADC_AVGCTRL_ADJRES(4) | ADC_AVGCTRL_SAMPLENUM_1024;                          // Average on 1024 samples. Expected time for avg: 750k/7/1024 = 104Hz = 9.6ms. Single conversion mode, single ended, 12bit
    while ((ADC->STATUS.reg & ADC_STATUS_SYNCBUSY) != 0);                                           // Wait for sync
